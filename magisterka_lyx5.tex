...
kolejnym punktem jest test z użyciem dwóch stanowisk. Nastąpi próba
połączenia przez protokół SSH, który nie jest natywnie wbudowany w
system Windows. Zostanie użyty program, który umożliwi uruchomienie
serwera oraz klienta protokołu SSH. Wybranym programem jest prosty
w obsłudze OpenSSH. 

BlenderVR wymaga by połączenie między komputerami nie wymagało hasła.
Oba stanowiska zostały wymienione kluczami SSH. Przy załadowaniu odpowiednio
napisanej konfiguracji nastąpi próba połączenia oraz uruchomienie
demona na obu stanowiskach. 

Podstawy pliku konfiguracyjnego pozostają te same: oba środowiska
uruchomieniowe posiadają identyczną architekturę. Dzięki temu wystarczy
jedynie dopisać dodatkowe komputery do ustawień wraz z ich adresami
IP. 
...
jednak wyświetliło błąd jak poprzednio: cannot start daemon for screen
dual. Po pierwsze sprawdzono czy połączenie SSH jest możliwe wraz
ze zdalnym uruchomieniem Pythona. Poprzez konsolę Windowsa udało się
bez problemów podłączyć do drugiego komputera. Problem nie leżał w
samym połączeniu. Raz jeszcze został zweryfikowany plik konfiguracyjny,
ale nie znaleziono w nim błędów. Próba wykonania tego samego polecenia
co aplikacje BlenderVR zwraca komunikat o nieistniejącym pliku. Podłoże
tego problemu najprawodpodobniej leży w ograniczeniach nałożonych
na konto, które nie ma praw administratora komputera. W laboratorium
konta dostępne do testów miały wystarczające uprawnienia by instalować
i uruchamiać większość programów znajdujących się poza dyskiem C (lub
pewnymi jego katalogami). Sesja SSH nie posiadała wszystkich zmiennych
środowiskowych oraz była uruchamiana w ograniczonym środowisku co
powodowało problemy ze zdalnym uruchomieniem skryptu demona. 
...
Windows. System operacyjny CentOS został uruchomiony i udostępniony
więc kolejne próby odbędą się w tym właśnie środowisku. Linux wspiera
natywnie protokół SSH więc etap uruchomienia dwóch stanowisk powinien
...
dostępu do Internetu. Każdy wymagany pakiet musi zostać dostarczony
z zewnątrz, skonfigurowany i zainstalowany. Konto użytkownika nie
miało praw super-user więc każdy z pakietów był instalowany do katalogu
domowego użytkownika, a następnie każdy został dodany do zmiennej
środowiskowej PATH.
...
i wiele wyników poprawnej konfiguracji jest na podstawie metody prób
i błędów. Wszystkie pakiety dostarczone były jako źródła oraz kompilowane
i instalowane na stanowiskach, ponieważ sieć odcięta była od Internetu. 
...
stanowiskach w laboratorium, dlatego pominięto ten krok w opisie. 
...
w instrukcji na stornie dewelopera i powoduje problemy przy kompilacji
...
Kolejnym wymaganym pakietem jest Qt4 wraz z development headers. Nagłówki
funkcji mogą być dostępne w osobnym pakiecie np. libqt4-dev. W takim
wypadku należy skompilować i zainstalować podstawowe biblioteki qt4
oraz nagłówki. Jest to wymagane przez pakiet PySide. Źródła Qt4 dostępne
...
1.2.2 – taką, jakiej zalecają deweloperzy. Pakiet został skompilowany
...
Został on napisany na przykładzie dostępnym na stronie dewelopera.
...
żadnych problemów i zakłóceń w działaniu sceny czy samej aplikacji.
\end{document}
