...
<\textcompwordmark{}<tutaj jakieś mocne i słabe strony blendervr,
problemy jakie moga wystąpić i co go wyróżnia. na co pozwala>\textcompwordmark{}>
...
Żaden z komputerów nie miał dostępu do Internetu. Wszystkie niezbędne
...
Następna faza testu sprawdzi czy aplikacja wyświetli obraz stereoskopiczny
...
3D pozwala na większe odświeżanie ekranu dzięki czemu można otrzymać
stabilne 60 klatek dla każdego oka oddzielnie. I w tym przypadku próba
zakończyła się sukcesem. Na monitorze ukazał się obraz, który po założeniu
okularów 3D sprawiał wrażenie trójwymiarowego.
...
Niestety konsola z informacjami wyświetliła błąd ,,Error: Cannot
start daemon for screen <screenName>''. Na stronie BlenderVR błąd
jest rozpoznany i według autora rozwiązania problem występuje jeśli
połączenie miedzy komputerami wymaga hasła. W tym przypadku jednak
nie ma to odniesienia ponieważ komputery zostały wymienione kluczami.
Połączenie SSH zostało przetestowane przez konsolę Windowsa i nie
wymagało użycia hasła. Po konsultacji jeden z twórców oprogramowania
polecił sprawdzić czy polecenie wykonywane przez BlenderVR można również
wykonać ręcznie przez konsolę. Ręczne wykonanie nie zwróciło jednak
żadnego błędu; komenda wykonała się poprawnie.

Problemem w tym przypadku może być brak <\textcompwordmark{}<natywnego
wsparcia>\textcompwordmark{}> protokołu SSH przez system operacyjny
Windows 7. Zewnętrzne narzędzia pozwalają na połączenie pomiędzy jednostkami
ale możliwe, że nie radzą sobie z prawami administratora. Logowanie
przez protokół na komputer podrzędny odbywało się przy użyciu konta
z prawami administratora. Zanim stanowiska zostały wymienione kluczami,
do logowania było wymagane hasło takie samo jak przy logowaniu użytkownika
na docelowym systemie. Teoretycznie rzecz biorąc po zalogowaniu się
przez SSH, użytkownik powinien mieć wszystkie prawa administratora. 
...
Test odbył się w pracowni komputerowej na Wydziale Matematyki, Fizyki
i Informatyki Uniwerytetu Gdańskiego. Zostały przygotowane dwa stanowiska 
\end{document}
