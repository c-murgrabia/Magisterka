...
obrazu na półprzezroczysty ekran znajdujący się między obserwatorem
...
przeznaczona dla lewego oka, praczona dla lewego oka, prawa migawka
pozostaje zamknięta i vice versa. Przy takiej formie odbierania obrazu
wyświetlacz powinien odświeżać obraz na tyle szybko by każde z oczu
otrzymywało przynajmniej 60 klatek na sekundę (co daje łącznie odświeżanie
na poziomie 120 Hz)\footnote{http://www.cnet.com/news/active-3d-vs-passive-3d-whats-better/ (dostęp:
22.05.2016)}
...
typu CRT odświeżają obraz na zasadzie ,,namalowania'' klatki na
ekranie po czym wygaszenie i proces się powtarza.
...
wirtualną.
\item \textbf{Stereoskopia }- technika pokazywania obrazu polegająca na
tworzeniu lub uwydatnianiu jego głębii.
\item \textbf{FullHD} - standard wyświetlania obrazu wysokiej jakości odznaczający
się 1080. horyzontalnymi liniami w rozdzielczości obrazu. Zazwyczaj
zakłada się też, że wyświetlacz jest panoramiczny (stosunek boków
jest 16:9) i posiada rozdzielczość 1920 na 1080. Sygnał wideo jakości
FullHD na oznaczenie 1080p.
...
wirtualnej rzeczywistości. Żaden z nich nie jest jednak programem
open-source co nie pozwala na dokładne dopasowanie do potrzeb laboratorium.
Wykorzystanie nowej opcji jaką jest Blender Game Engine rozszerzy
możliwości modelowania scen dedykowanych dla CAVE’a. 
...
To co dzisiaj kryje się pod nazwą rzeczywistości wirtualnej zostało
rozpopularyzowane dopiero przez media masowe w filmach takich jak:
,,Brainstorm'' i ,,The Lawnmover Man''. Pierwszym urządzeniem,
które można by nazwać systemem VR była Sensorama\footnote{http://www.theverge.com/a/virtual-reality/oral\_history (dostęp 07.05.20016)}.
Była to maszyna podobna wyglądem do automatów do gier wyświetlająca
stereoskopiczny obraz 3D, zapewniała przechylanie pozycji użytkownika
...
Forte VFX-1. Gogle mogły wyświetlać stereoskopiczny obraz, miały wbudowane
...
przy pomocy różnych kontrolerów np. FlySticka lub tzw. różdżki. Są
to swego rodzaju bezprzewodowe joysticki wyposażone w zestaw czujników
potrafiących określić pozycje (jako zestaw trzech wektorów) oraz orientacje
(podawaną jako kwaternion) urządzenia w przestrzeni. Oprócz tego kontroler
wyposażony jest w podstawowe części takie jak: analogowy, dwuosiowy
joystick oraz kilka przycisków i/lub trigger. Przykładowy model został
pokazany na obrazku (Fig. 3).
...
\section{Implementacja BlenderVR w środowisku wirtualnej jaskini CAVE}
...
dowolną liczbę ekranów. BlenderVR posiada wsparcie dla wyświetlania
obrazu 3D (poczwórne buforowanie - ang: Quad Buffering) i protokołów
komunikacyjnych np. VRPN – virtual reality peripherial network lub
OSC – open sound control.

--tutaj jakieś mocne i słabe strony blendervr, problemy jakie moga
wystąpić i co go wyróżnia. na co pozwala.--
...
\item Ustawienia wtyczki interakcji ze sceną: serwer odbierający dane z
urządzeń wskazujących oraz konfiguracja czujników.
\end{itemize}
...
„Run” znajduje się start programu oraz konsola do debugowania.
...
\item Kompilacja i ustawienie wtyczki do obsługi kontrolerów – VRPN
...
\subsection{Testy aplikacji w Laboratorium Zanurzonej Wizualizacji Przestrzennej}

W piątek 6. maja odbyły się pierwsze testy aplikacji BlenderVR. Jednostka,
na której odbywały się testy została wyposażona w procesor klasy Intel
Xeon, kartę graficzną Nvidia Quadro oraz monitor ASUS 24'' w pełni
obsługujący wyświetlanie obrazu 3D. Aplikacja oraz wszystkie niezbędne
komponenty musiały zostać dostarczone na zewnętrznym nośniku ponieważ
komputer nie został podłączony do Internetu. 

Pierwsza próba instalacji i uruchomienia odbyła się w środowisku systemu
Linux. Jednakże brak dostępu do Internetu sprawił, że niektóre biblioteki
pakietów Pythona oraz Qt4.8 nie mogły zostać pobrane (jak również
był problem z dostarczeniem odpowienich plików i zależności z zewnątrz).
Kolejna próba odbyła się w środowisku Windows. Instalator automatyczny,
stworzony przez deweloperów BlenderVR, zawierał prawie kompletny pakiet
Pythona3.4 oraz kompletny pakiet bibliotek Qt4.8. Problemem na tej
platformie również okazał się brak dostępu do Internetu. Brakowało
pakietów Pythona o nazwie PySide oraz Virtualenv. Te jednak udało
się dostarczyć i zainstalować.

Aplikacja została uruchomiona. Przystąpiono więc do pierwszych testów
przykładowych scen, dołączonych do programu. Podstawowa konfiguracja
programu wymagała podania ścieżki do plików wykonywalnych blender.exe
oraz blenderplayer.exe. Okazało się, że program Blender3D nie może
być zainstalowany na dysku C: (tak, jak było to domyślnie ustawione)
z powodu braku wystarczających uprawnień użytkownika. Usunięcie oraz
zainstalowanie ponownie aplikacji Blender3D na dysku D: wyeliminowało
problem braku uprawnień. 

Pierwsza próba wyświetlenia sceny przykładowej przeprowadzona została
w konfiguracji 2D oraz wyświetlaniu w oknie 400x400 pikseli. Po załadowaniu
odpowiedniego pliku konfiguracyjnego oraz ustawień ekranu na monitorze
pokazało się okno z przykładową sceną. Sukces.

Kolejna próba wykorzystywała już możliwości 3D monitora. Odpowiednio
zmodyfikowany plik konfiguracyjny: Stereo 3D Quad Buffer oraz odświeżaniu
120Hz. Tyle wynosi odświeżanie aktywnych okularów 3D (po 60Hz na każde
oko, obraz wyświetlany naprzemiennie)\footnote{http://blender-vr-manual.readthedocs.io/components/configuration-file.html\#desktop-stereo-3d
(dostęp 07.05.20016)}. Tutaj wystąpił problem bibliotek Visual C++ Redistributable, który
udało się dość szybko wyeliminować poprzez instalację odpowiedniego
pakietu. Próba ostatecznie zakończyła się sukcesem a na monitorze
został wyświetlony stereoskopiczny obraz dopasowany do aktywnych okularów
3D. 

Kolejną próbą w laboratorium LZWP była próba nawiązania połączenia
przez sieć między dwoma komputerami. BlenderVR posługuje się protokołem
komunikacyjnym SSH. Dzięki temu program jest w stanie na podanych
komputerach uruchomić wybraną scenę w BGE. W laboratorium zakładano
testy w środowisku Linux (ponieważ protokół jest tam wbudowany) jednakże
pewne problemy techniczne podczas uruchamiania systemu uniemożliwiły
przeprowadzenie testu. Na systemie operacyjnym Windows do wykorzystania
protokołu ssh potrzebujemy programu OpenSSH for Windows. Umożliwia
on użycie z poziomu konsoli polecenia ,,ssh''. Po udanym zainstalowaniu
serwera oraz klienta protokołu SSH z powodzeniem nawiązano połączenie.
Następnym krokiem będzie rekonfiguracja BlenderVR tak, by można było
wykorzystać go do uruchomienia przykładowej sceny BGE na drugim komputerze.
\end{document}
